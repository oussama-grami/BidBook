...
export class BookDetailsComponent implements OnInit, OnDestroy {
      ...
      this.querySubscription = this.bookService.getBookDetails(this.bookId).subscribe({
        next: (book: Book) => {
          if (book) {
            ...
            this.owner = book.owner || undefined;
      ...
      this.favoriteActionSubscription = this.bookService.addBookToFavorites(this.currentUserId, this.bookId).subscribe({
        next: (success) => {
          if (success) {
            ...
            console.warn('Add favorite mutation did not return success=true. Reverting optimistic update.');
      ...
      this.favoriteActionSubscription = this.bookService.removeFavorite(this.currentUserId, this.bookId).subscribe({
        next: (success) => {
          if (success) {
            ...
            console.warn('Remove favorite mutation did not return success=true. Reverting optimistic update.');
  ...
  submitComment(): void {
    ...
    this.bookService.addCommentToBook(this.bookId, this.currentUserId, commentContent).pipe(
      map(response => response.data?.addCommentToBook)
    )
    .subscribe({
      next: (newComment) => {
        if (newComment) {
